...
export function reduceRun(previous,event){
 ...
 switch(event.type){
  ...
  case 'done':r.elapsed=event.elapsed_ms||0;set('report',r.report?'done':'error',r.report?'智能体结果已返回':'未返回有效分析结果');break;
 }
 return r;
}
